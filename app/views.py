...
def view_Customer(request):
    ...
    return render(request, 'Customer_Temp/view_Customer.html', {'data': data})

def view_Customer_admin(request):
    data=Login.objects.filter(is_customer=True)
    return render(request,'Admin_Temp/view_Customer_admin.html',{'data':data})
...
def edit_customer(request,id):
    ...
    return render(request,'Customer_Temp/edit_customer.html',{'form':form})
...
def view_Dealer_custom(request):
    data=Login.objects.filter(is_dealer=True)
    return render(request,'Customer_Temp/view_Dealer_custom.html',{'data':data})
...
#     data=Feedback,object.filter(id=id)
#     if request.method=='POST':
...
def bill_payment(request,id):
    data = Payment.objects.filter(id=id)
    if request.method=='POST':
        ...
        print(exp)
        creditcard(card_no=card,card_cvv=cvv,expiry_date=exp,Payment=data).save()
        data.status=1
        data.save()
...
def bill_history(request):
    ...
    return render(request,'Customer_Temp/bill_history.html',{'data':data})